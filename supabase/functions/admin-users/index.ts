...
Deno.serve(async (req) => {
  ...
  if (action === "set_role") {
    const { userId: targetUserId, role, remove } = body;
    if (!targetUserId || !role) return new Response(JSON.stringify({ error: "Missing userId or role" }), { status: 400, headers: corsHeaders });

    if (remove) {
      await adminClient.from("user_roles").delete().eq("user_id", targetUserId).eq("role", role);
    } else {
      await adminClient.from("user_roles").upsert({ user_id: targetUserId, role }, { onConflict: "user_id,role" });
    }
